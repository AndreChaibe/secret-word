...
const Game = ({ currentWord, setWord, setActive, score, randomWord }) => {
    ...
    const correctLetters = useMemo(() =>
        guessesLetter.filter(guess =>
            lettersWordSecret.map(l => l.toLocaleUpperCase()).includes(guess)
        ),
        [guessesLetter, lettersWordSecret]
    );

    const incorrectLetters = useMemo(() =>
        [...new Set(guessesLetter)].filter(letter =>
            !currentWord.word.toLocaleUpperCase().includes(letter)
        ),
        [guessesLetter, currentWord.word]
    );

    const hasWon = useMemo(() => {
        const uniqueCorrect = new Set(correctLetters).size;
        const uniqueWord = new Set(lettersWordSecret.map(letter => letter.toLocaleUpperCase())).size;
    ...
    useEffect(() => {
        if (hasWon) {
            const timer = setTimeout(() => {
                score.setUserScore((prevScore) => prevScore + 100);
                setGuessesLetter([]);
                setLifes(3);
                setWord(randomWord());
            }, 500);
    ...
    }, [hasWon, score, setWord, randomWord]);

    function handleUserLetter(event) {
        const value = event.target.value.toLocaleUpperCase();

        if (value && value !== " ")  setUserLetter(value.slice(0, 1));
        else setUserLetter("");
    }

    function handleLossLife() {
        if (!currentWord.word.toLocaleUpperCase().includes(userLetter)) {
    ...
    function handleGuess(event) {
        ...
        if (userLetter) {
            const upperLetter = userLetter.toLocaleUpperCase();

            if (!guessesLetter.includes(upperLetter)) {
                handleLossLife();
                setGuessesLetter([...guessesLetter, upperLetter]);
            }
            setUserLetter("");
        }
    }
    ...
    function renderGuessesIncorrects() {
        if (incorrectLetters.length > 0) {
            return incorrectLetters.map((letter, index) => {
                return (
                    <li className="bg-red-500 p-1.5 rounded-lg" key={index}>
                        {letter}
                    </li>
                );
            });
        ...
        <div className="flex flex-col justify-center text-center items-center w-full text-2xl gap-10 max-[593px]:gap-7">
            <div className="flex flex-col text-center gap-3 text-[1.4rem]">
                <motion.h1
                    ...
                    className="text-5xl font-semibold mb-3 max-[593px]:text-4xl"
                >
                    Adivinhe a Palavra
                </motion.h1>
                ...
                >
                    <p>Dica sobre a palavra: <span>{currentWord.tip}</span></p>
                    <p>Você ainda tem <span>{lifes}</span> tentativa(s)</p>
                    <p>Pontuação: <span>{score.userScore}</span></p>
                </motion.div>
            </div>
            ...
            <motion.p
                ...
                animate={{ opacity: 1, scale: 1 }}
                transition={{ duration: 0.5, ease: "easeOut" }}
                className="bg-black/40 py-2 px-6 rounded-2xl max-[380px]:text-[1.2rem]"
            >
                Tente adivinhar uma letra da palavra
            </motion.p>

            <div className="flex justify-center items-center gap-6 [&>input]:text-black [&>input]:text-[2.5rem] [&>input]:leading-none [&>input]:text-center">
            ...
            <div className="absolute bottom-10 right-10 flex flex-row gap-5 justify-end items-center max-[1047px]:static max-[380px]:text-[1.2rem]">
